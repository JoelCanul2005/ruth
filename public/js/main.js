document.addEventListener('DOMContentLoaded', function() {
    ...
    if (form) {
      form.addEventListener('submit', function(event) {
        ...
        var buttonNino = document.createElement('button');
        buttonNino.innerText = 'Niño';
        buttonNino.onclick = function() {
          registrarUsuario(email, password, 'nino');
        };
        modalContent.appendChild(buttonNino);
  ...
  function registrarUsuario(email, password, role) {
    ...
    .then(data => {
      if (data.success) {
        if (role === 'adulto') {
          window.location.href = `/RegistroAdultos.html?userId=${data.userId}`;
        } else if (role === 'profesional') {
          window.location.href = `/RegistroProfesional.html?userId=${data.userId}`;
        } else if (role === 'nino') {
          window.location.href = `/RegistroNino.html?userId=${data.userId}`;
        }
      } else {
